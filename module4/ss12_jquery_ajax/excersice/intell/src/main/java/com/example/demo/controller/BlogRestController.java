...
public class BlogRestController {
    ...
    public ResponseEntity<Page<Blog>> getListBySearch(@PathVariable("search") String search,@PageableDefault(page = 0, size = 3)Pageable pageable) {
        Page<Blog> blogList = blogService.findByProducerBlogContaining(search,pageable);
        if (blogList.isEmpty()) {
            return new ResponseEntity<>(HttpStatus.NO_CONTENT);
        }
