...
//    @Transactional
//    @Modifying
    @Query(value = "update customer c set c.flag_delete = false where c.id = :id",countQuery="update customer c set c.flag_delete = false where c.id = :id", nativeQuery = true)
    void removeFlag(@Param("id") Long id);

    Page<Customer> findByFlagDelete(boolean flagDelete,Pageable pageable);
}
