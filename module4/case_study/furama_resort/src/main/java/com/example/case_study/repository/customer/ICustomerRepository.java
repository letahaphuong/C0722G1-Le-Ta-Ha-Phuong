...
//    Page<CustomerView> searchView(@Param("name") String name, @Param("email") String email, @Param("customerTypeId") String customerTypeId, Pageable pageable); OKKKKKK
    ...
    @Query(value = "select c.id, c.customer_type_id as customerTypeId, c.name ,c.date_of_birth as dateOfBirth,c.phone_number as phoneNumber,c.email,c.gender,ct.name as customerTypeName from customer as c join customer_type as ct on c.customer_type_id = ct.id where  c.flag_delete  = true and (c.name like concat('%',:name,'%') and c.email like concat('%',:email,'%') and c.customer_type_id like concat ('%',:customerTypeId,'%') )"
